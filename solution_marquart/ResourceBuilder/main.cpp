...
int main(int argc, char** argv)
{
	...
	buildDefinitions["diffuse"].Command = sol_dir + "../../external/terrain_tools/ged-texture-converter/nBuildDrv.exe -verbose -2 -p \"Input=%d\" -p \"Output=" + out_dir + "%f.ntx\" -p \"Input-Type=Color\" \"BuildTools.TextureImporter\"";
	buildDefinitions["diffuse"].Output = "%f.ntx";

	buildDefinitions["specular"].Comparer = ".png,spec";
	buildDefinitions["specular"].Command = sol_dir + "../../external/terrain_tools/ged-texture-converter/nBuildDrv.exe -verbose -2 -p \"Input=%d\" -p \"Output=" + out_dir + "%f.ntx\" -p \"Input-Type=Color\" \"BuildTools.TextureImporter\"";
	buildDefinitions["specular"].Output = "%f.ntx";

	buildDefinitions["glow"].Comparer = ".png,glow";
	buildDefinitions["glow"].Command = sol_dir + "../../external/terrain_tools/ged-texture-converter/nBuildDrv.exe -verbose -2 -p \"Input=%d\" -p \"Output=" + out_dir + "%f.ntx\" -p \"Input-Type=Color\" \"BuildTools.TextureImporter\"";
	buildDefinitions["glow"].Output = "%f.ntx";

	buildDefinitions["normal"].Comparer = ".png,normal";
	buildDefinitions["normal"].Command = sol_dir + "../../external/terrain_tools/ged-texture-converter/nBuildDrv.exe -verbose -2 -p \"Input=%d\" -p \"Output=" + out_dir + "%f.ntx\" -p \"Input-Type=XY-Normal\" \"BuildTools.TextureImporter\"";
...
unsigned long command(std::string cmdline, std::string path)
{
	...
	while(cmd.find("%f")!=cmd.npos) cmd = cmd.replace(cmd.find("%f"),2,path.substr(path.find_last_of('\\')+1,path.find_last_of('.')-path.find_last_of('\\')-1));
