	...
	float fElapsedTime, void* pUserContext )
{
	...
	{
		g_MeshRenderer->AddToRenderPass(&o); //Render Shadow
	}
	...
	g_Effect_VLS->GetVariableByName("heightMap")->AsShaderResource()->SetResource(g_TerrainRenderer->getTerrainHeightSRV());
	...
	g_Effect_VLS->GetVariableByName("lightPosition")->AsVector()->SetFloatVector(g_SkyboxRenderer->GetSunPosition());
	g_Effect_VLS->GetVariableByName("timer")->AsScalar()->SetFloat(fTime);
	g_Effect_VLS->GetVariableByName("terrainDim")->AsScalar()->SetFloat(*(g_TerrainRenderer->getDepth()));
	...
	if(useDeveloperFeatures)
	{
		...
		g_Effect->GetVariableByName("g_VLSMap")->AsShaderResource()->SetResource(screenRTV->GetShaderResource());
